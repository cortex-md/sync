...
func TestBroker_FinishFlushRetainsBufferOnFailure(t *testing.T) {
	b := collab.NewBroker(10, 0)
	vaultID := uuid.New()
	filePath := "doc.md"
	...
	require.NoError(t, b.BufferUpdate(vaultID, filePath, []byte{0xAA}))
	pending := b.BeginFlush(vaultID, filePath)
	require.Len(t, pending, 1)

	b.FinishFlush(vaultID, filePath, len(pending), false)

	flushed := b.FlushUpdates(vaultID, filePath)
	require.Len(t, flushed, 1)
	assert.Equal(t, []byte{0xAA}, flushed[0])
}
