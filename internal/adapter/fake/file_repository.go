...
func (r *FileLatestRepository) ListByPaths(_ context.Context, vaultID uuid.UUID, filePaths []string) ([]domain.FileLatest, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	result := make([]domain.FileLatest, 0, len(filePaths))
