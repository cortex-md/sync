...
func (r *FileLatestRepository) ListByPaths(ctx context.Context, vaultID uuid.UUID, filePaths []string) ([]domain.FileLatest, error) {
	if len(filePaths) == 0 {
		return nil, nil
	}
	rows, err := connFromContext(ctx, r.pool).Query(ctx,
		`SELECT vault_id, file_path, current_version, latest_snapshot_version, checksum, size_bytes, content_type, deleted, last_modified_by, last_device_id, updated_at, created_at
		 FROM file_latest WHERE vault_id = $1 AND file_path = ANY($2)`,
		vaultID, filePaths,
	)
	if err != nil {
		return nil, mapError(err)
	...
	byPath := make(map[string]domain.FileLatest, len(filePaths))
	for rows.Next() {
		var f domain.FileLatest
	...
	result := make([]domain.FileLatest, 0, len(byPath))
	for _, filePath := range filePaths {
		if f, ok := byPath[filePath]; ok {
