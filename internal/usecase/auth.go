...
func (uc *AuthUsecase) SetRegistrationMode(mode string) {
	if mode == "" {
		mode = "open"
	}
	uc.registrationMode = mode
}

func (uc *AuthUsecase) SetDevOpsNotifier(notifier port.DevOpsNotifier) {
	uc.devops = notifier
}
...
func (uc *AuthUsecase) Register(ctx context.Context, input RegisterInput) (*RegisterOutput, error) {
	...
	if uc.registrationMode == "closed" {
		return nil, domain.ErrRegistrationClosed
	}
	if uc.registrationMode == "first-user" {
		userCount, err := uc.users.Count(ctx)
		if err != nil {
			return nil, err
		}
		if userCount > 0 {
	...
	uc.notifyDevOps(ctx, port.DevOpsEvent{
		...
		Fields: map[string]string{
			"registration_mode": uc.registrationMode,
		},
	})
...
func (uc *AuthUsecase) Login(ctx context.Context, input LoginInput) (*LoginOutput, error) {
	...
	revokedDeviceReactivated := false
	deviceReassigned := false
	previousUserID := uuid.Nil
	if err != nil {
		if !errors.Is(err, domain.ErrNotFound) {
			return nil, err
		}
		...
		if device.Revoked {
			revokedDeviceReactivated = true
			device.Revoked = false
		}
		if device.UserID != user.ID {
			deviceReassigned = true
	...
	if deviceReassigned {
		uc.notifyDevOps(ctx, port.DevOpsEvent{
			...
			Fields: map[string]string{
				...
				"previous_user_id": previousUserID.String(),
			},
		})
	}
	if revokedDeviceReactivated {
		uc.notifyDevOps(ctx, port.DevOpsEvent{
			Type:        "auth.revoked_device_reactivated",
			UserID:      user.ID,
			Email:       user.Email,
			DisplayName: user.DisplayName,
			OccurredAt:  time.Now(),
...
func (uc *AuthUsecase) notifyDevOps(ctx context.Context, event port.DevOpsEvent) {
	if uc.devops == nil {
		return
	}
